...
- **Visual**: Manage advanced visual settings, such as ASCII art style, animations, gradients (requires PS7+), and border aesthetics.
- **Sounds**: Toggle various interface sound effects for events such as startup, errors, and task completion.
...
- v3 uses the same config file path as v2 (`%APPDATA%\WindowsUpdateManager\config.json`).
- Visual and Sounds configuration properties are automatically added with safe defaults on first load.
- No data loss - your old configuration items remain intact.
